...
exports.createComplaint = async (req, res) => {
  try {
    const { booking_id, subject, description, priority } = req.body;

    if (!subject || subject.trim() === '') {
      return res.status(400).json({ message: 'Complaint subject is required' });
    }
    ...
    await Promise.all(admins.map(admin => Promise.all([
      ...
      notify(
        ...
        `Guest "${booking.guest?.name}" submitted a complaint "${subject.trim()}" (priority: ${priority || 'low'}) for booking #${booking_id}.`,
        'complaint_new',
