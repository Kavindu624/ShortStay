...
const { hostRespondedToReviewEmail } = require('../utils/emailTemplates');
...
exports.respondToReview = async (req, res) => {
  try {
    ...
    // Load guest user for email
    const guest = await User.findByPk(review.booking.guest_id);

    // Email notification → guest
    if (guest) {
      await sendEmail(
        guest.email,
        'Host Replied to Your Review - ShortStay',
        hostRespondedToReviewEmail(
          guest.name,
          review.booking.property.title,
          req.body.response
        )
      );
    }

    // In-app notification → guest (Feature 2)
    await notify(
